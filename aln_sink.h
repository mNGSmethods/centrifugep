...
struct SpeciesMetrics {
                   ...
                   const EList<size_t>& len)
    {
		 ...
		 * date: 2022/9
		 * edited by Ma Yunran
		 * Add variables to be used to calculate pj-value
		 */
        map<uint64_t,double> pj;
        map<uint64_t,double> uniqreads;
        map<uint64_t,double> nonuniqreads;
            ...
            for(size_t i = 0; i < ids.size(); i++) {
                ...
                if(id_itr != tid_to_num.end()) {
                    ...
                    for(size_t c = 0; c < children.size(); c++) {
                        ...
                        if(ids.size() == 1){
                            uniqreads[c_num] += count;
                        } else {
                            nonuniqreads[c_num] += count;
                        }
                    }
                }
            }
        ...
        for(map<IDs, uint64_t>::const_iterator itr = observed.begin(); itr != observed.end(); itr++) {
            ...
            for(size_t i = 0; i < ids.size(); i++) {
                ...
                if(id_itr != tid_to_num.end()) {
					...
					// p_next[num] += (pj[num] * (p[num] * len[num] / psum));
                    pj[num] = (uniqreads[num] / (uniqreads[num] + nonuniqreads[num]));
                    if(ids.size() == 1){
                        p_next[num] += (pj[num] * count * (p[num] * len[num] / psum));
                    } else {
                        double uniqpj = 1.0 - pj[num];
                        p_next[num] += (uniqpj * count * (p[num] * len[num] / psum));
                    }
                } else {
                    map<uint64_t, EList<uint64_t> >::const_iterator a_itr = ancestors.find(tid);
                    if(a_itr == ancestors.end())
                    ...
                    for(size_t c = 0; c < children.size(); c++) {
                        ...
                        pj[c_num] = uniqreads[c_num] / (uniqreads[c_num] + nonuniqreads[c_num]);
                        if(ids.size() == 1){
                            p_next[c_num] += (pj[c_num] * count * (p[c_num] * len[c_num] / psum));
                        } else {
                            double uniqpj = 1.0 - pj[c_num];
                            p_next[c_num] += (uniqpj * count * (p[c_num] * len[c_num] / psum));
                        }
                    }
                }
            }
